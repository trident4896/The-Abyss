...
public class MMReticle : MonoBehaviour, ICardboardPointer {
	...
	private const float kReticleDistanceMin = 0f;
	...
	void Start () {
		...
		reticleDistanceInMeters = kReticleDistanceMin;
		gazeAt = null;

		if (count == 2) {
			mainMenu.text = "Game Over";
		}
	}
	...
	public void OnGazeStart(Camera camera, GameObject targetObject, Vector3 intersectionPosition) {
		...
		if (count == 0) {
			mainMenu.text = "Press to Play";
		} else if (count == 1) {
			mainMenu.text = "Try Again";
		} else if (count == 2) {
			mainMenu.text = "Play Again?";
		}
		gazeAt = targetObject;
	}
